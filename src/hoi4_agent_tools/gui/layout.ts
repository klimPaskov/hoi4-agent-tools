...
interface LayoutGeometry {
  ...
  interactionBlock?: Exclude<NonNullable<GuiSceneElement['disabledReason']>, 'hidden'>;
...
function refreshSceneClips(context: LayoutContext): void {
  for (const element of context.output) {
    ...
    element.visibilityReason = element.visible
      ? 'visible'
...
async function layoutAttachedScrollbars(
  ...
  const attachmentRect = owner.unclippedRect;
  const attachmentWidth = viewport.width > 0 ? attachmentRect.width : 0;
  const attachmentHeight = viewport.height > 0 ? attachmentRect.height : 0;
  ...
  for (const [index, attached] of scrollbars.entries()) {
    ...
    const coordinate = (horizontal: boolean) => {
      ...
      const explicit =
        rowValues?.[key] ?? context.scenario.values[key] ?? context.scenario.scriptedGui[key];
      const span = horizontal ? attachmentWidth : attachmentHeight;
      const offset =
      ...
      return (
        anchoredCoordinate(horizontal ? attachmentRect.x : attachmentRect.y, span, offset, anchor) -
...
async function layoutElement(
  ...
  const interactionBlock = !interactive
    ? undefined
    : clickThrough
      ? ('click_through' as const)
      : scriptedEnabled === false
        ? ('scripted_enabled_false' as const)
        : requestedState === 'disabled' || requestedState === 'locked'
  ...
  context.geometry.set(instanceId, {
    ...
    ...(interactionBlock === undefined ? {} : { interactionBlock }),
    clipChain,
    contentRect,
