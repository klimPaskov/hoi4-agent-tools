...
import { boundedSourceHashEvidence, publicArtifactLink } from './artifacts.js';
import type { ArtifactLink, ServiceResult } from './result.js';
import { ServiceError } from './result.js';
import type { TransactionManifest } from './transactions.js';
import { PACKAGE_VERSION } from '../version.js';
import { postValidateTransaction } from './domain-validation.js';
...
export interface TransactionExecutionHooks {
  /** Persist a background request's journal binding before any source write begins. */
  beforeApply?: (transaction: Readonly<TransactionManifest>) => Promise<void>;
}
...
export async function executePlannedTransaction(
  ...
  principal?: string,
  signal?: AbortSignal,
  hooks: TransactionExecutionHooks = {},
  ...
  await hooks.beforeApply?.(transaction);
  signal?.throwIfAborted();
    ...
    const applied = await engine.transactions.apply(
      ...
      transaction.planHash,
      {
        ...
        postValidate: async (manifest, validationSignal) => {
          ...
          return { ...validation, artifacts: executionArtifacts };
        },
      },
    );
