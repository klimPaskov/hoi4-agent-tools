...
export const artifactResourceDefinition = {
  name: 'artifact',
  uriTemplate: 'hoi4-agent://workspace/{workspaceId}/artifact/{sha256}/{provenanceHash}/{name}',
...
export type ArtifactResourceContent = {
  uri: string;
  mimeType: string;
  _meta: Record<string, unknown>;
} & ({ text: string; blob?: never } | { blob: string; text?: never });

/** Shared bounded resource reader. URI ownership never supplies a principal or root grant. */
export async function readArtifactResource(
  engine: CoreEngine,
  uriInput: string,
  context: OperationContext,
  signal: AbortSignal,
): Promise<{ contents: ArtifactResourceContent[] }> {
  signal.throwIfAborted();
  let uri: URL;
  let workspaceId: string;
  try {
    if (uriInput.length > 8192) throw new Error('URI exceeds the resource limit');
    ...
    throw new ServiceError('RESOURCE_URI_INVALID', 'Resource URI is invalid');
  }
  signal.throwIfAborted();
  ...
  if (metadata !== null && metadata !== 'manifest') {
    throw new ServiceError('ARTIFACT_METADATA_INVALID', 'Artifact metadata selector is invalid');
  }
  if (metadata === 'manifest') {
    const described = await engine.artifacts.describe(workspace, contentArtifactUri(uri), signal);
    ...
    const selected = manifestBytes.subarray(start, Math.min(manifestBytes.length, start + length));
