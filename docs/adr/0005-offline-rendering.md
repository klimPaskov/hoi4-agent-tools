...
HOI4 bitmap fonts are composed from their declared BMFont metrics and atlas channels. Native metrics, glyph contours, and decoded coverage samples are preserved through layout without per-glyph normalization. Native-size coverage is unchanged; enlarged coverage uses deterministic Lanczos2 interpolation, and downscaling uses nearest-neighbour sampling. No synthetic sharpening, thresholding, opacity gain, or edge reconstruction is applied. Bitmap placements snap to the output pixel grid so the compositor cannot introduce fractional-position blur.
