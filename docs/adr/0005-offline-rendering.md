...
HOI4 bitmap fonts are composed from their declared BMFont metrics and atlas channels. Native metrics, glyph contours, and decoded coverage samples are preserved through layout without per-glyph normalization. Scaling uses nearest-neighbour coverage without synthetic sharpening, thresholding, or edge reconstruction, and bitmap placements snap to the output pixel grid so the compositor cannot introduce fractional-position blur.
