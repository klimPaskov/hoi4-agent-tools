...
CI uses only project-owned synthetic workspaces: a 255-focus tree, a 170-element scripted GUI, and a nontrivial 24-bit BMP mini-world. Test layers cover unit, source round-trip, property, schema, golden artifact, image comparison, bitmap diff, transaction fault/recovery, security/isolation, both MCP transports, Inspector, package installation, and Registry validation.

Local opt-in tests read configured installed-game and external-mod roots without copying or modifying them. Performance reports cover cold and warm scans/layouts/renders.
