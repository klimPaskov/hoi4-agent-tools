...
CI uses only project-owned synthetic workspaces: a 255-focus tree, a 170-element scripted GUI, a nontrivial 24-bit BMP mini-world, an event workspace with more than 300 definitions and deterministic route defects, a 1,040-technology workspace, and an analyzer workspace with more than 150 weighted blocks and 250 declared scenarios. Test layers cover unit, source round-trip, arithmetic and property identities, uncertainty bounds, seeded sampling, sequence transitions, schema, golden artifact, graph topology, event state and scope flow, image comparison, bitmap diff, transaction fault/recovery, security/isolation, both MCP transports, Inspector, package installation, and Registry validation.
