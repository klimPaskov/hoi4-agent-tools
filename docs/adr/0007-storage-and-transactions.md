...
Publish artifact contents and provenance manifests through complete same-directory temporary files followed by exclusive hard-link creation. No process can observe a partially written immutable artifact or manifest, and concurrent processes producing the same content verify and reuse the completed file. Storage scans ignore private publication temporaries and tolerate files that another process legitimately removed after directory enumeration. Enumeration skips invalid or foreign manifests without exposing their content. Admission isolates malformed, oversized, stale-identity, and address-mismatched manifests instead of allowing one generated record to disable unrelated routes. A matching deterministic write may replace an invalid manifest only from freshly recomputed expected bytes; a self-consistent different manifest at the same address remains a collision and fails closed. Direct reads still verify the complete immutable address and current workspace identity.
