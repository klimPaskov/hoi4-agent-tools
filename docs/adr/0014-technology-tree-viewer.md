...
Technology identity remains separate from its folder-placement instances. Folder views use source coordinates and GUI gridbox geometry, including file-local numeric constants and declared gridboxes for multi-root technologies. Their artwork comes from the source item templates and folder sprites: state skins, icon and name positions, decoded background dimensions, and the center tile of a cornered panel sprite. Dependency and semantic views are labelled as generated analysis. `sourceAccurate` describes pixel placement and does not assert visual equivalence to a game viewport. JSON is authoritative, and complete or focused graphs, folder views, SVG, PNG, optional HTML, comparisons, diagnostics, and provenance use the existing MCP artifact resource.
