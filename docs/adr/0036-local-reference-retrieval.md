...
Context result limits reserve one installed-documentation citation and one wiki citation before further required-source pointers and question matches. A one-result request prioritizes installed documentation. Each required source contributes its best question-matching section when available, avoiding a redundant title-only pointer to the same file. `omittedSources` distinguishes required sources excluded by the limit from `missing` sources absent from the workspace. This additive result field preserves existing inputs, source identities, revision checks, and read continuations.
