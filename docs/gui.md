...
`uiScale` is the in-game UI scale, not an image zoom. Use `1` for the normal 1920×1080 layout. Enlarged bitmap-font glyphs are pre-scaled from their real atlas with deterministic pixel sampling, and bitmap colours use alpha masks so local and VPS renders produce the same pixels.
