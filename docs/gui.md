...
`uiScale` is the in-game UI scale, not an image zoom. Use `1` for the normal 1920×1080 layout. Enlarged bitmap-font glyphs retain their real atlas contours while reconstructing the atlas's native one-pixel antialias edge at the target pixel density; this prevents soft edge pixels from widening with the scale. Bitmap placement is snapped to the output pixel grid, and bitmap colours use alpha masks, so native, fractional-scale, local, and VPS renders remain sharp and deterministic.
