...
The active older installation and coding-agent processes were not restarted or replaced.
Release commit `ba598fa3a326f7e54bfce2b2b4069942887f9f84` passed CI run `35776400446` on Windows and Linux with Node 22 and 24, including coverage, the official MCP Inspector, both transports, container checks, package checks, and Registry validation.
...
At v3.4.1 publication, GitHub retained every historical version tag and one public release entry, v3.4.1; no pull request or feature branch remained.
...
The sampled chemical and biological folder renders place 19 of 19 and 8 of 8 current-source nodes, respectively, and resolve every requested sprite. Both current-source folders use square technology cards. The supplied chemical viewport matches an older source revision with nine wide equipment cards; the source-matched MCP render uses the 131×52 cylinder artwork at native size. Seven sampled technology card interiors from supplied game captures showed normalized grayscale correlations from 0.998712 to 0.999589 without resizing. Those scores measure selected interiors, not complete folder images or the wide-card presentation. The [visual comparison](../visual-comparisons.md) records the source revisions and remaining presentation differences.
